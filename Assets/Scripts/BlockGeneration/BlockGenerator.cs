...
#region reference
///https://github.com/Szuszi/CityGenerator-Unity/blob/main/CityGenerator2D/Assets/Procedural%20City%20Generator/Scripts/BlockGeneration/BlockGenerator.cs
#endregion
namespace BlockGeneration
{
    public class BlockGenerator
    {
        ...
        public List<Block> UnoccupiedBlocks { get; private set; }
        public List<Block> ThinnedBlocks { get; private set; } // After sidewalk application
        public List<BlockNode> ThinnedBlockNodes { get; private set; }
        ...
        public BlockGenerator(Graph graphToUse, int mapSize, float majorThickness, float minorThickness, float blockHeight)
        {
            ...
            UnoccupiedBlocks = new List<Block>();
            ThinnedBlocks = new List<Block>();
            ThinnedBlockNodes = new List<BlockNode>();
        ...
        private void CreateBlocks() //Connects the BlockNodes into one Block
        {
        ...
        private void DeleteMapEdgeBlocks()
        {
            ...
            {
                Blocks.Remove(block);
                UnoccupiedBlocks.Add(block);
            }
        }
        #endregion

        #region Block Thinning (*For sidewalk implimentation)
        // refractor of original extension/thinning method
        public void AddSidewalks(float sideWalkThickness)
        {
            foreach (var block in Blocks)
            {
                var thinnedBlock = GetThinnedBlock(block, sideWalkThickness);
                ThinnedBlocks.Add(thinnedBlock);
                foreach (BlockNode node in thinnedBlock.Nodes) { ThinnedBlockNodes.Add(node); }
            }
        ...
        #region Node Thickening/Extension Specific Cases (*doesn't alter edges yet but copies original to needed amount)
